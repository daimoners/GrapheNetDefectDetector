...
def objective(trial):
    ...
    alpha = 0.5
    composite_score = alpha * mean_val_rmse + (1 - alpha) * (1 - mean_r2) + 0.4 * overfit_quality
    logging.info(f"Trial - Mean Train RMSE: {mean_train_rmse}, Mean Val RMSE: {mean_val_rmse}, Composite Score: {composite_score}, Params: {params}")
...
with open('single_objective_results_with overfit.json', 'w') as f:
